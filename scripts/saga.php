...
foreach($xml->grups->grup as $grup){
	...
	$alumnes = $ldap->generateMultipath($alumnes, "uidNumber", $ldap->path("ou=Alumnes,ou=Users", TRUE));
...
$profes_id = $ldap->generateMultipath($profes_id, "uidNumber", $ldap->path("cn=Professors,ou=Users"), TRUE);

echo $ldap->createGroupOfNames("Pares", $pares_id, ["description" => "Pares que poden accedir a la aplicacio de gestio."]);
echo $ldap->createGroupOfNames("Professors", $profes_id, ["description" => "Profes que poden accedir a la aplicacio de gestio."]);
